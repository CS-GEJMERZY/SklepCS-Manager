    ...
    public partial class SklepcsManagerPlugin
    {
        ...
        public void OnBuyWalletCommand(CCSPlayerController? player, CommandInfo commandInfo)
        {
            HandlePlayerCommand(player, commandInfo, () =>
            {
                ...
                if (commandInfo.ArgCount >= 1 && int.TryParse(commandInfo.GetArg(1), out int planID))
                {
                    ...
                    Models.ServicePlanData? data = WebManager!.GetService(planID);
                    ...
                    Task.Run(async () =>
                    {
                        ...
                        if (success)
                        {
                            Server.NextFrame(() =>
                            {
                                ...
                                if (Config.Settings.IsLoggingLevelEnabled(Models.LoggingLevelData.PurchaseSuccess))
                                {
                        ...
                        else
                        {
                            Server.NextFrame(() =>
                            {
                                ...
                                if (Config.Settings.IsLoggingLevelEnabled(Models.LoggingLevelData.PurchaseErrors))
                                {
        ...
        public void OnShopBuyCommmand(CCSPlayerController? player, CommandInfo commandInfo)
        {
            HandlePlayerCommand(player, commandInfo, () =>
            {
                ...
                if (commandInfo.ArgCount >= 1 && int.TryParse(commandInfo.GetArg(1), out int planId))
                {
                    Models.ServicePlanData? service = WebManager!.GetService(planId);
                    if (service == null)
                    {
                        player!.PrintToChat($"{PluginChatPrefix}{Localizer["sklepsms.how_to_buy_sms"]}");
                        return;
